...
def generate_historical_data():
    ...
    if f'rodada-{rodada}.csv' not in os.listdir('data/real'):
        ...
        print('Rodada já inclusa em data/real. Terminando execução') 


def generate_preview_data():
    df_atletas = ef.gerar_df_atletas()
    rodada = df_atletas[c.RODADA_ID][0] + 1
    if f'rodada-{rodada}.csv' not in os.listdir('data/previews'):
        ...
        df_rodada_predictions.to_csv(f'data/previews/rodada-{rodada}.csv', index=False)
    else:
        print('Previsão de rodada já inclusa em data/previews. Terminando execução') 
